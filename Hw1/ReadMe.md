...
* #### conclusion：
  ...
  因為少了transition所以計算上少了很多麻煩，只要知道角度跟縮放量就能計算出結果。discussion內的重點有注意到大概就能算出來了。差值法那邊用雙線性差值反而會變模糊，所以就簡單用around。小心圖片中心不一樣就好。 然後 題目的pdf p.11 Rotation 右邊的公式寫錯了。
  補：旋轉那邊要再加條件，旋轉超過180會有問題(還未改)
